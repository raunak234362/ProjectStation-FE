...
const WBTDashboard = () => {
  ...
  const tasksToUse = useMemo(
    ...
    [userType, departmentTask, taskData]
  );
  ...
  const taskStats = useMemo(
    () => ({
      ...
      inProgress:
        tasksToUse?.filter((task) => task?.status === "IN_PROGRESS")?.length ||
        0,
      assigned:
  ...
  const projectStats = useMemo(
    () => ({
      ...
      completed:
        projectData?.filter((p) => p.status === "COMPLETE")?.length || 0,
  ...
  const lineData = useMemo(
    () => ({
      labels:
        projectsWithTasks?.map((project) => project?.name).filter(Boolean) ||
        [],
      datasets: [
        {
          label: "Tasks Completed",
