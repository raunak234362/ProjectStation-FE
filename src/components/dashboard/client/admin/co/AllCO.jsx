...
const AllSentCO = () => {
  ...
  const fetchSentCO = async () => {
    try {
      ...
      console.log("ccccccccccccccccooooooooooooo", response?.data);
    } catch (error) {
  ...
  console.log(projectInfo, "lllllllllllllll")

  const handleSearch = (e) => {
    const term = e.target.value;
    setSearchTerm(term);
  };
  //search by term and filter data
  ...
  console.log("Filtered CO:", filteredCO);
  const handleClose = () => {
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        Header: "Actions",
        Cell: ({ row }) => (
          <Button onClick={() => handleClick(row.original.id)}>View</Button>
        ),
      },
    ],
    []
      ...
      <div className="p-4">
        <div className="max-h-[600px] overflow-auto">
          ...
          <div className="w-full mb-4">
            <input
              type="text"
              placeholder="Search by Change order"
              value={searchTerm}
