...
const ProjectStatus = ({ projectId, onClose }) => {
  ...
  const [sortBy, setSortBy] = useState("stage_date") // New sorting option

  console.log("Project Stage:", filterStage)
  ...
  const filteredTaskData = useMemo(() => {
    if (userType === "department-manager") {
      const departmentProjects = taskData.filter((task) => task.id === projectId)
  ...
  const applyDateFilter = useCallback(
    (data) => {
      ...
      return data.filter((task) => {
        ...
        // Date range filter (specific start and end dates)
        if (dateFilter.type === "dateRange" && dateFilter.startDate && dateFilter.endDate) {
          ...
          const filterEndDate = new Date(dateFilter.endDate + "T23:59:59.999Z").getTime() // make end inclusive
  ...
  const filteredGanttData = useMemo(() => {
    ...
    if (filterStage !== "all") {
      filtered = filtered.filter((project) => {
        const originalTask = projectTasks.find((p) => p.id === project.id)
        console.log("Original Task:", originalTask)
        return originalTask && originalTask.stage === filterStage
      })
    }
      ...
      <div className="bg-white h-[90vh] overflow-y-auto p-4 md:p-6 rounded-lg shadow-lg w-11/12 md:w-10/12">
        ...
        <div className="mb-6 border-b">
          <div className="flex overflow-x-auto">
            ...
            <button
              className={`px-4 py-2 font-medium text-sm whitespace-nowrap ${
                activeTab === "team" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-900"
              }`}
            ...
            <button
              className={`px-4 py-2 font-medium text-sm whitespace-nowrap ${
                activeTab === "RFI" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-900"
              }`}
            ...
            <button
              className={`px-4 py-2 font-medium text-sm whitespace-nowrap ${
                activeTab === "CO" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600 hover:text-gray-900"
              }`}
