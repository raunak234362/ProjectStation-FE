...
const GroupDetail = ({ group, onClose }) => {
  ...
  const [isAddGroupMemberModalOpen, setIsAddGroupMemberModalOpen] =
    useState(false);
  ...
  useEffect(() => {
    const options = staffData
      .map((staff) => {
        const label = `${staff?.f_name || ""} ${staff?.m_name || ""} ${
          staff?.l_name || ""
        }`.trim();
  ...
  const handleMembersSubmit = async (data) => {
    ...
    try {
      ...
      fetchGroupMembers(groupId); // Refresh members list after adding
  ...
  const handleDeleteMember = async (memberId) => {
    ...
    try {
      const response = await Service.deleteMemberById(memberId, groupId);
      console.log("Member deleted:", response);
      fetchGroupMembers(groupId); // Refresh members list after deletion
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-85 flex justify-center items-center z-50">
      <div className="bg-white h-fit p-5 md:p-5 rounded-lg shadow-lg w-11/12 md:w-3/12 ">
        ...
        <div className="flex flex-col items-center mt-4">
          ...
          <ul className="list-disc list-inside">
            {members?.map((member) => (
              ...
              >
                <span>
                  {member.members?.f_name} {member.members?.m_name}{" "}
                  {member.members?.l_name}
                </span>
                {userType === "admin" && (
                  <Button
        ...
        {userType === "admin" && (
          <div>
            <Button
              className="mt-4 bg-teal-500 px-2 py-1"
              onClick={() => handleAddGroupMemberClick(groupId)}
            >
              Add Group Member
            </Button>
          </div>
          ...
          <div className="bg-white h-[80vh] p-5 md:p-5 rounded-lg shadow-lg w-11/12 md:w-6/12 ">
            <div className="flex flex-row justify-between">
              <Button
                ...
                onClick={handleAddGroupMemberClose}
              >
                Close
              </Button>
            </div>
