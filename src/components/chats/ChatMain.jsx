...
import DefaultChatBG from "../../assets/CHATBG.webp";
import ForNiloy from "../../assets/girlforniloy.jpeg"
...
const ChatMain = ({ activeChat, setActiveChat }) => {
      ...
      if (response) {
        const chatData = response.map((chat) => ({
          ...
          senderName:
            chat.senderId !== userInfo?.id
              ? `${chat.sender?.f_name} ${chat.sender?.l_name}`
              : null,
        }));
        ...
        if (chatData.length > 0) {
          ...
          setCurrentConversation((prev) => {
            ...
            const newMessages = reversedChatData.filter(
    ...
    const handleIncomingMessage = (msg) => {
      if (msg.groupId === activeChat?.group?.id) {
        ...
        const newMessage = {
          ...
          senderName: (() => {
            const sender = staffData?.find(
              (staff) => staff.id === msg.senderId
            );
            return sender
              ? `${sender.f_name} ${sender.l_name}`
              : "Unknown Sender";
          })(),
        };
  ...
  const username = sessionStorage.getItem("username") || "WBT-140";
  ...
  return (
    ...
    >
      ...
      <div
        className="flex-1 overflow-y-auto p-4 bg-green-50/60"
      ...
      <div className="sticky bottom-0 z-0 bg-white border-t border-gray-200 p-3">
        ...
        >
          ...
          <Button
            ...
            className="bg-teal-500 text-white hover:bg-teal-600 rounded-full p-2"
          >
            <Send size={24} />
          </Button>
        </form>
