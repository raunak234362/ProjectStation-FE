...
const EditRFI = ({ rfi, onUpdate, onClose }) => {
    const [loading, setLoading] = useState(false);
    const [joditContent, setJoditContent] = useState(rfi?.description || "");
    const [files, setFiles] = useState([]);

    const clientData = useSelector((state) => state?.fabricatorData?.clientData);
    const fabricatorID = rfi?.fabricator_id || rfi?.project?.fabricatorID;
    ...
    const onSubmit = async (data) => {
        ...
        formData.append("recipient_id", rfi.recipient_id);
        formData.append("subject", data.subject);
        formData.append("description", data.description);

        try {
            setLoading(true);
            const response = await Service.updateRFI(rfi.id, formData);
            ...
            toast.error("Error updating RFI");
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white h-[60%] overflow-y-auto p-5 rounded-lg shadow-lg w-full md:w-6/12">
                <div className="flex justify-between mb-5 bg-teal-200/50 p-2 rounded-lg">
                    <h2 className="text-2xl font-bold">Edit RFI</h2>
                    <button
