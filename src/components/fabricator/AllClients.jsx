...
const AllClients = ({ fabricator }) => {
    ...
    const columns = useMemo(
        () => [
            ...
            {
                Header: "Client Name",
                accessor: (row) => `${row?.f_name || ''} ${row?.m_name || ''} ${row?.l_name || ''}`.trim(),
                id: "clientName",
            },
            {
                Header: "Phone",
    ...
    return (
        <div className="bg-white md:w-full w-[90vw] my-4">
            <div className="bg-white p-5 overflow-x-auto">
                <table {...getTableProps()} className="w-full border-collapse text-center text-sm md:text-base rounded-xl">
                    <thead>
                    ...
                    <tbody {...getTableBodyProps()}>
                        {rows.length === 0 ? (
                            ...
                            rows.map((row) => {
                                prepareRow(row);
                                return (
                                    <tr {...row.getRowProps()} className="hover:bg-gray-100 border">
                                        {row.cells.map((cell) => (
