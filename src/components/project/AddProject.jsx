...
const AddProject = () => {
    ...
    const managerOption = userData
        ...
        ?.map((user) => ({ label: `${user.f_name} ${user.l_name}`, value: user.id }));
            ...
            <div className="w-full h-full py-3 px-3 overflow-y-auto ">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <CustomSelect
                        ...
                        options={fabricatorData?.map((fab) => ({ label: fab.fabName, value: fab.id }))}
                    ...
                    <Input type="textarea" label="Description" {...register("description", { required: true })} />
                    <Input type="number" label="Estimated Hours" min="0" {...register("estimatedHours", { required: true })} />
                    ...
                    <CustomSelect
                        label="Status"
                        name="status"
                        options={["ASSIGNED", "ACTIVE", "ONHOLD", "INACTIVE", "DELAY", "COMPLETE"].map(s => ({ label: s, value: s }))}
                    ...
                    <CustomSelect
                        ...
                        name="stage"
                        options={[
                            "RFI", "IFA", "BFA", "BFA_M", "RIFA", "RBFA", "IFC", "BFC", "RIFC", "REV", "CO#"
                    ...
                    <CustomSelect
                        label="Department"
                        options={departmentData?.map(dep => ({ label: dep.name, value: dep.id }))}
                    ...
                    <CustomSelect
                        label="Tools"
                        options={["TEKLA", "SDS2", "PEMB"].map(tool => ({ label: tool, value: tool }))}
                        {...register("tools")}
                        onChange={setValue}
                    />
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        <Input type="date" label="Start Date" {...register("start_date", { required: true })} />
                        <Input type="date" label="Approval Date" {...register("approvalDate", { required: true })} />
                    </div>
