...
const AllProjects = () => {
  ...
  const [filters, setFilters] = useState({ fabricator: "", status: "", manager: "" });
  ...
  const managerOptions = useMemo(() => {
    const managers = new Set(
      projects?.map((project) => project?.manager?.f_name).filter((name) => name)
    );
  ...
  const filterAndSortData = useCallback(() => {
    let filtered = projects?.filter((project) => {
      ...
      const fabricatorMatch = !filters.fabricator || project?.fabricator?.fabName === filters.fabricator;
      const statusMatch = !filters.status || project?.status === filters.status;
      const managerMatch = !filters.manager || project?.manager?.f_name === filters.manager;
    ...
    filtered.sort((a, b) => {
      ...
      return sortOrder.order === "asc" ? aValue > bValue ? 1 : -1 : aValue < bValue ? 1 : -1;
    });
    ...
    <div className="w-full p-4 rounded-lg bg-white/70">
      ...
      <div className="flex flex-col gap-4 mb-4 md:flex-row">
        ...
        >
          ...
          {fabricators?.map((fab) => (
            <option key={fab.id} value={fab.fabName}>{fab.fabName}</option>
          ))}
        </select>
        <select
          name="status"
