...
const ProjectStatus = ({ projectId, onClose }) => {
    ...
    const ganttData = useMemo(() => {
        return projectTasks.map((task) => {
            const startDate = task.start_date && !isNaN(new Date(task.start_date)) ? new Date(task.start_date) : new Date();
            const endDate = task.due_date && !isNaN(new Date(task.due_date)) ? new Date(task.due_date) : new Date();
            const duration = Math.ceil((endDate - startDate) / (1000 * 60 * 60 * 24));
            const type = task.name?.split(" ")[0] || "Unknown";

            const assignedHours = parseDuration(task.duration);
            const takenHours = task.workingHourTask?.reduce((sum, innerTask) => sum + (Number(innerTask.duration) || 0), 0) || 0;
            ...
            else if ((task.status === "IN_PROGRESS" || task.status === "BREAK") && progress > 80) progress = 80;
            ...
            return data.filter((task) => {
                ...
                try {
                    ...
                    if (dateFilter.type === "month" && dateFilter.year !== undefined && dateFilter.month !== undefined) {
                        ...
                        const filterEndDate = new Date(dateFilter.year, dateFilter.month + 1, 0, 23, 59, 59, 999).getTime();
                        return (
                    ...
                    if (dateFilter.type === "year" && dateFilter.year !== undefined) {
                        ...
                        const filterEndDate = new Date(dateFilter.year, 11, 31, 23, 59, 59, 999).getTime();
                        return (
                            (taskStartDate >= filterStartDate && taskStartDate <= filterEndDate) ||
                            (taskEndDate >= filterStartDate && taskEndDate <= filterEndDate) ||
                            (taskStartDate <= filterStartDate && taskEndDate >= filterEndDate)
                    ...
                    if (
                        ...
                        const filterEndDate = new Date(dateFilter.year, dateFilter.endMonth + 1, 0, 23, 59, 59, 999).getTime();
                        return (
                            (taskStartDate >= filterStartDate && taskStartDate <= filterEndDate) ||
                            (taskEndDate >= filterStartDate && taskEndDate <= filterEndDate) ||
                            (taskStartDate <= filterStartDate && taskEndDate >= filterEndDate)
                    ...
                    if (dateFilter.type === "dateRange" && dateFilter.startDate && dateFilter.endDate) {
                        const filterStartDate = new Date(dateFilter.startDate).getTime();
                        const filterEndDate = new Date(dateFilter.endDate).setHours(23, 59, 59, 999);
                        return (
                            (taskStartDate >= filterStartDate && taskStartDate <= filterEndDate) ||
                            (taskEndDate >= filterStartDate && taskEndDate <= filterEndDate) ||
                            (taskStartDate <= filterStartDate && taskEndDate >= filterEndDate)
    ...
    const filteredGanttData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(
                (task) =>
                    task.name.toLowerCase().includes(lowerSearchTerm) || task.username.toLowerCase().includes(lowerSearchTerm)
            );
        }
        ...
        if (filterStage !== "all") {
            filtered = filtered.filter((task) => {
                ...
                return originalTask && (originalTask.stage || originalTask.Stage || originalTask.STAGE || "").toUpperCase() === filterStage.toUpperCase();
            });
        }
        ...
        return filtered;
    }, [ganttData, searchTerm, filterStatus, filterType, filterUserId, filterStage, applyDateFilter, projectTasks]);

    // Gantt chart props
    const minDate = useMemo(() => {
        return projectTasks.length > 0
            ? Math.min(...projectTasks.map((task) => new Date(task.start_date).getTime()))
            : new Date().getTime();
    }, [projectTasks]);

    const maxDate = useMemo(() => {
        return projectTasks.length > 0
            ? Math.max(...projectTasks.map((task) => new Date(task.due_date).getTime()))
            : new Date().getTime();
    }, [projectTasks]);
    ...
    const typeColors = useMemo(() => {
        const types = [...new Set(projectTasks.map((task) => (task.name || "").split(" ")[0] || "Unknown"))];
                ...
                {!loading && !error && projectData && (
                    <>
                        ...
                        {activeTab === "projectDetail" && (
                            <GetProject projectId={projectId} onClose={onClose} projectData={projectData} />
                        )}
                        {activeTab === "overview" && (
                            <Overview
                                projectData={projectData}
