...
const Overview = ({ projectData, projectTasks, userContributions, dateFilter, filterStage }) => {
                ...
                taken: tasks.reduce(
                    (sum, task) =>
                        ...
                        (task?.workingHourTask?.reduce(
                            (innerSum, innerTask) => innerSum + (Number(innerTask.duration) || 0),
                            0
                        ) || 0),
                    0
    ...
    const stageStats = useMemo(() => {
        ...
        const stageHours = {
            assigned: filteredTasks.reduce((sum, task) => sum + parseDuration(task.duration), 0),
            taken: filteredTasks.reduce(
                (sum, task) =>
                    sum +
        ...
        return {
            ...
            efficiency: stageHours.taken > 0 ? ((stageHours.assigned / stageHours.taken) * 100).toFixed(1) : 0,
            completedTasks: filteredTasks.filter((task) => task.status === "COMPLETE").length,
            inProgressTasks: filteredTasks.filter((task) => task.status === "IN_PROGRESS").length,
            inReviewTasks: filteredTasks.filter((task) => task.status === "IN_REVIEW").length,
        };
    ...
    return (
        <div className="space-y-6">
            ...
            {loading && <div className="p-4 bg-blue-50 text-blue-700 rounded-lg">Loading data...</div>}
                    ...
                    <div className="p-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            ...
                            <div>
                                ...
                                <span className="font-semibold">{formatHours(stageStats.assignedHours)}</span>
                            </div>
                            <div>
                                <span className="text-gray-600">Time Taken:</span>{" "}
                                <span className="font-semibold">{formatHours(stageStats.takenHours)}</span>
                            </div>
                            <div>
                                <span className="text-gray-600">Completed:</span>{" "}
                                <span className="font-semibold text-green-600">{stageStats.completedTasks}</span>
                            </div>
                        </div>
                    </div>
                </Card>
                ...
                {[
                    {
                        ...
                        subtext: filterStage && filterStage !== "all" ? `In ${getStageDisplayName(filterStage)}` : null,
                    },
                    {
                        icon: <ClipboardCheck className="w-5 h-5 text-green-700" />,
                        label: "Completed",
                        value: filteredTasks.filter((task) => task.status === "COMPLETE").length,
                        component: <ProgressBar value={filteredTasks.filter((task) => task.status === "COMPLETE").length} max={filteredTasks.length} />,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        subtext: filterStage && filterStage !== "all" ? "For selected stage" : null,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        value: totalTakenHours > 0 ? `${Math.round((totalAssignedHours * EFFICIENCY_FACTOR) / totalTakenHours * 100)}%` : "0%",
                        subtext: filterStage && filterStage !== "all" ? "Stage efficiency" : null,
                    },
                ...
                <Card className="bg-gradient-to-br from-teal-50 to-teal-100">
                    <div className="p-5">
                        ...
                        <div className="space-y-3 text-sm">
                            {[
                                { label: "Total Tasks", value: filteredTasks.length,className: "text-teal-800"  },
                                { label: "Completed", value: filteredTasks.filter((task) => task.status === "COMPLETE").length, className: "text-teal-800" },
                                { label: "In Review", value: filteredTasks.filter((task) => task.status === "IN_REVIEW").length, className: "text-teal-800" },
                                { label: "In Break", value: filteredTasks.filter((task) => task.status === "BREAK").length, className: "text-teal-800" },
                                { label: "In Progress", value: filteredTasks.filter((task) => task.status === "IN_PROGRESS").length, className: "text-teal-800" },
                                { label: "Not Started", value: filteredTasks.filter((task) => task.status === "ASSIGNED").length, className: "text-teal-800" },
                            ...
                            {filterStage && filterStage !== "all" && stageStats && (
                                <div className="pt-2 border-t border-blue-200">
                                    {[
                                        { label: "Modeling", value: ((workBreakdownHours.MODELING?._sum?.totalExecHr || 0) / 60).toFixed(2) },
                                        { label: "Model Checking", value: ((workBreakdownHours.MODELING?._sum?.totalCheckHr || 0) / 60).toFixed(2) },
                                        { label: "Detailing", value: ((workBreakdownHours.DETAILING?._sum?.totalExecHr || 0) / 60).toFixed(2) },
                                        { label: "Detail Checking", value: ((workBreakdownHours.DETAILING?._sum?.totalCheckHr || 0) / 60).toFixed(2) },
                                        { label: "Erection", value: ((workBreakdownHours.ERECTION?._sum?.totalExecHr || 0) / 60).toFixed(2) },
                                        { label: "Erection Checking", value: ((workBreakdownHours.ERECTION?._sum?.totalCheckHr || 0) / 60).toFixed(2) },
                                        { label: "Stage Efficiency", value: `${stageStats.efficiency}%`, className: "text-purple-600" },
                                    ].map(({ label, value, className }, index) => (
                                        <div key={index} className="flex items-center justify-between">
                                            <span className="text-gray-600">{label}:</span>
                                            <span className={`font-semibold ${className || "text-gray-500"}`}>{value} {label.includes("Efficiency") ? "" : "hours"}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
